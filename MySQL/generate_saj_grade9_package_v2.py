"""
Generate SAJ Grade 9 Practice Package with question format from raw_to_JSON/output
Matches questions from Quiz Bank Grade 9 to the 30 topics in the kisi-kisi document.
Uses the enhanced JSON format with id_options, en_options, and SVG images.
"""
...
OUTPUT_JSON = "JSON_Exports/SAJ_Grade9_Practice_Package_v2.json"
OUTPUT_MD = "MD_Exports/SAJ_Grade9_Practice_Package_v2.md"
...
def convert_to_enhanced_format(source_question, question_number):
    """Convert source question to enhanced format with id_options, en_options"""
    options = source_question.get('options', [])
    ans = source_question.get('ans', 0)
    
    # Create options array (A, B, C, D labels)
    option_labels = ['A', 'B', 'C', 'D', 'E']
    formatted_options = option_labels[:len(options)]
    
    # Split options into Indonesian and English versions
    id_options = []
    en_options = []
    
    for opt in options:
        # For bilingual questions, try to detect language
        # Simple heuristic: if contains English words, put in en_options
        opt_lower = opt.lower()
        english_indicators = ['students', 'salary', 'distance', 'speed', 'time', 'triangle', 'circle']
        
        is_english = any(ind in opt_lower for ind in english_indicators)
        
        if is_english:
            en_options.append(opt)
        else:
            id_options.append(opt)
    
    # If all options are same (bilingual question has same options)
    if not id_options:
        id_options = options.copy()
    if not en_options:
        en_options = options.copy()
    
    # Generate SVG placeholder if image exists
    image_svg = ""
    if source_question.get('image'):
        # Use the image URL directly or generate a simple SVG placeholder
        image_url = source_question.get('image', '')
        if image_url.startswith('http'):
            # Create a simple SVG placeholder that references the image
            image_svg = f"<svg width='200' height='150' viewBox='0 0 200 150' xmlns='http://www.w3.org/2000/svg'><rect width='200' height='150' fill='#f0f0f0'/><text x='100' y='70' font-family='Arial' font-size='14' text-anchor='middle' fill='#666'>Question Image</text><text x='100' y='95' font-family='Arial' font-size='10' text-anchor='middle' fill='#999'>{image_url[:50]}...</text></svg>"
        else:
            image_svg = source_question.get('image', '')
    
    formatted_question = {
        "id": question_number,
        "id_q": source_question.get('id_q', ''),
        "en_q": source_question.get('en_q', ''),
        "image": image_svg,
        "options": formatted_options,
        "id_options": id_options,
        "en_options": en_options,
        "ans": ans,
        "id_exp": source_question.get('id_exp', ''),
        "en_exp": source_question.get('en_exp', ''),
...
def generate_practice_package():
    ...
    for kisi in KISI_KISI:
        ...
        if question:
            ...
            # Format the question with topic info
            formatted_question = {
                "id": kisi['no'],
                "topic": kisi['topic'],
            ...
            enhanced_question = convert_to_enhanced_format(formatted_question, kisi['no'])
            selected_questions.append(enhanced_question)
            ...
            selected_questions.append({
                ...
                "image": "",
                "options": ["A", "B", "C", "D"],
                "id_options": ["Opsi A", "Opsi B", "Opsi C", "Opsi D"],
    ...
    # Save JSON output
    output_data = {
        "title": "SAJ Grade 9 Practice Package v2",
        "description": "Practice questions based on Kisi-Kisi SAJ Kelas 9 (Enhanced Format)",
    ...
    md_content = "# 📚 SAJ Grade 9 Practice Package v2\n\n"
    md_content += "**Based on Kisi-Kisi Bank Soal SAJ Kelas 9**\n\n"
    md_content += "**Format:** Enhanced JSON with bilingual options\n\n"
    ...
    for category in sorted(categories.keys()):
        ...
        for q in categories[category]:
            ...
            if q['image']:
                md_content += f"![Image]({q['image'] if q['image'].startswith('http') else 'data:image/svg+xml;base64,...'})\n\n"
            
            md_content += "**Options (ID):**\n"
            for i, opt in enumerate(q.get('id_options', q['options'])):
                md_content += f"- {chr(65+i)}. {opt}\n"
            md_content += "\n"
            
            md_content += "**Options (EN):**\n"
            for i, opt in enumerate(q.get('en_options', q['options'])):
                md_content += f"- {chr(65+i)}. {opt}\n"
            md_content += "\n"
            
            ans_label = chr(65+q['ans']) if isinstance(q['ans'], int) else q['ans']
